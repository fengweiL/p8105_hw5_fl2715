...
For the samples where the null was rejected, the average estimate of ($\hat{\mu}$) tends to be slightly higher than the true value when true mean is equal to 1, 2, and 3. The average estimate of ($\hat{\mu}$) is equal to the true mean when true mean is equal to 0, 4, 5, and 6. This is due to selection bias: tests are more likely to reject when sample means are further from zero, creating an upward bias in the average estimate when conditioning on rejection.
